...
import {
  advanceUntil, answerRealTurn, breakDawn, byChar, endDayByConsensus, fastForwardToVote, markAllReady, mk as baseMk, mkDay as baseMkDay, runFullNight, skipRound, startNight, voteInOrder,
} from './helpers.js';
 ...
 */
function pin<T extends GameState>(s: T): T {
  s.players.forEach((p, i) => { p.id = `pid-${i}`; });
  return s;
}
const mk = (chars: CharacterId[], opts: Parameters<typeof baseMk>[1] = {}): GameState => pin(baseMk(chars, opts));
const mkDay = (chars: CharacterId[]): GameState => pin(baseMkDay(chars));

/** Gives the players the names used in the wiki example (in seat order). */
function named<T extends GameState>(s: T, names: string[]): T {
...
function findSecret(attempt: (secret: string) => boolean, what: string, tries = 3000): string {
...
function afterNight1(layout: CharacterId[], opts: Parameters<typeof mk>[1] = {}): GameState {
...
type Pick = CharacterId | PlayerState;
type Picks = Partial<Record<CharacterId, Pick[]>>;
const idOf = (s: GameState, x: Pick): string => (typeof x === 'string' ? byChar(s, x).id : x.id);

/** Plays the night that is in progress: named steps answered with the named characters, others skipped. */
function playSteps(s: GameState, picks: Picks): void {
  let guard = 0;
  while (s.pendingRealTurn && guard++ < 60) {
    const p = picks[s.pendingRealTurn.charId as CharacterId];
    if (p) answerRealTurn(s, p.map((c) => idOf(s, c)));
    else skipRound(s);
  }
  breakDawn(s);
}
 ...
 */
function untilStep(s: GameState, until: CharacterId, impTarget: CharacterId): void {
  let guard = 0;
...
function night(s: GameState, picks: Picks): void {
  startNight(s);
  playSteps(s, picks);
}

/** Everyone votes yes on `nominee` (nominated by `nominator`), then the day ends: an execution. */
function execute(s: GameState, nominator: PlayerState, nominee: PlayerState): void {
...
const lastInfo = (p: PlayerState) => p.log.at(-1)!.msg;
const others = (s: GameState, self: PlayerState) => s.players.filter((p) => p.id !== self.id);
const pairOf = (m: { vars?: Record<string, unknown> }) => [m.vars!.a, m.vars!.b].sort();
